...
import type { LandlordReport } from "@/lib/types";
...
/**
 * Vista de Reportes. `initialReport` viene del Server Component (fetch en servidor
 * con la cookie httpOnly): si está presente, se pinta sin waterfall. Si es `null`
 * (sin sesión de servidor, p. ej. en e2e), cae al fetch de cliente habitual.
 */
export function ReportesView({
  ...
  const load = useCallback((m: string) => {
    api.getLandlordReport(landlordId, m)
      .then((report) => setResult({ month: m, report, error: null }))
      .catch((e: Error) => setResult({ month: m, report: null, error: e.message }));
  }, [landlordId]);

  useEffect(() => {
    // Solo pedimos al cliente si aún no tenemos el reporte de ese mes (el del
    // servidor ya cubre la carga inicial).
    if (result?.month !== month) load(month);
