...
| **G10** | **Recuperación de contraseña sin backend.** El UI expone `/recuperar-contrasena` y `/nueva-contrasena`. Las funciones `forgotPassword(email)` y `resetPassword(token, password)` llaman a `POST /auth/forgot-password` y `POST /auth/reset-password` respectivamente; esas rutas **aún no existen en el backend**. Hasta que se implementen, el flujo mostrará siempre el estado de éxito (la página de "revisa tu correo"), pero ningún correo se envía realmente. | `src/app/recuperar-contrasena/page.tsx`, `src/app/nueva-contrasena/page.tsx`, `api.ts` (`forgotPassword`, `resetPassword`) | **Backend:** implementar `POST /auth/forgot-password` (`{ email }` → `void 200`, no revelar si el email existe), `POST /auth/reset-password` (`{ token, password }` → `void 200`, `400/404` si token inválido o expirado), y el envío de correo con el enlace `https://<host>/nueva-contrasena?token=<jwt>`. |
